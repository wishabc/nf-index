...
vsd <- vst(dds, blind = F)
# FIXME, add option to do vst without reading the file first
df <- dispersionFunction(dds)
saveRDS(df, file=paste(args[5], ".vst.params.RDS", sep='')) 
rm(df)
rm(dds)
gc()
np$save(paste(args[5], ".vst.npy", sep=''), np$array(assay(vsd)))
rm(vsd)
gc()
...
dds <- estimateDispersions(dds)
df <- dispersionFunction(dds)
saveRDS(df, file=paste(args[5], ".no_sf.vst.params.RDS", sep='')) 

rm(dds2)
gc()
np$save(paste(args[5], ".no_sf.vst.npy", sep=''), np$array(assay(vsd2)))
